...
public static class DollMinionCmd
{
    ...
    public static async Task<Creature> SummonCopy(PlayerChoiceContext choiceContext, Player player, DollMinion origin, CardModel? cardSource)
    {
        ...
        // applier 設為 null 避免受到玩家能力加成影響（如力量加成、災厄加成等）
        await PowerUtils.ApplyPowersDynamically(choiceContext, newMinion, powers, null, cardSource);
        return newMinion;
    }
    ...
    {
        var creatures = GetAllDollMinions(player).Select(minion => minion.Creature).ToList();
        decimal maxHp = creatures.Sum(c => c.MaxHp);
        decimal hp = creatures.Sum(c => c.CurrentHp);
        List<PowerModel> powers = [.. creatures.SelectMany(c => c.Powers).Where(p => p.Type == PowerType.Buff)];

        foreach (var c in creatures)
        {
            // TODO: 是否要用殺死這件事情有待商榷，可能連鎖其他 Minion 死亡效果會有意外情況？
